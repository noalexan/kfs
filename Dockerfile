...
RUN apt install -y build-essential flex bison pkg-config m4 libtool automake autoconf autopoint gettext texinfo libfreetype-dev gawk xorriso

RUN tar xvf /binutils-2.43.tar.gz -C /
WORKDIR /binutils-2.43
RUN ./configure --prefix=/tools --target=i386-linux-gnu && make && make install

RUN tar xvf /gcc-14.2.0.tar.gz -C /
WORKDIR /gcc-14.2.0
RUN tar xvf /gmp-6.3.0.tar.gz && ln -sv gmp-6.3.0 gmp && tar xvf /mpc-1.3.1.tar.gz && ln -sv mpc-1.3.1 mpc && tar xvf /mpfr-4.2.1.tar.gz && ln -sv mpfr-4.2.1 mpfr
