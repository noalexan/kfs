...
{
	uint32_t total_count = BOOT_ALLOC_FREE_COUNT(alloc) + 
						BOOT_ALLOC_RESERVED_COUNT(alloc) + 
...
void *boot_alloc(uint32_t size)
{
    ...
    for (int i = BOOT_ALLOC_FREE_COUNT(&bootmem) - 1; i >= 0; i--) {
        ...
        if (region_size >= size) {
            ...
            boot_allocator_add_region(&bootmem, (uintptr_t)ret, 
                                     (uintptr_t)ret + size, RESERVED_MEMORY);
